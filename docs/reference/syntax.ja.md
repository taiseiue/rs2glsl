...
`match` は整数型（`i32`/`u32`）に対して使えます。GLSL の `switch` 文に変換されます。

```rust
let diffuse = match material_id {
    0 => lambertian(n, l),
    1 => blinn_phong(n, l, v),
    _ => 0.0,
};
...
| `<`, `>`, `<=`, `>=` | 大小比較 |
| `&&`, `\|\|` | 論理 AND / OR |
| `!` | 論理否定（`bool`）/ ビット反転（`i32`/`u32`） |

### ビット演算子

`i32` と `u32` に対して使えます。GLSL のビット演算子に直接変換されます。
...
両辺は同じ整数型（`i32` 同士または `u32` 同士）である必要があります。

```rust
fn pcg_hash(state: u32) -> u32 {
    let s = state * 747796405u32 + 2891336453u32;
    let w = (s >> ((s >> 28u32) + 4u32)) ^ s;
    w * 277803737u32
}

fn flags_test(flags: i32) -> bool {
    flags & 0x01 != 0
}
```
...
| オペランド型 | GLSL 出力    |
|-------------|--------------|
| `f32`, `Vec2/3/4` | `mod(a, b)` |
| `i32`, `u32` | `a % b`   |

```rust
let tiled = uv % 1.0;          // → mod(uv, 1.0)
let wrapped = index % count;   // → (index % count)  ※ i32/u32 のとき
```
...
```rust
let flipped = -uv;
let outside = !inside;         // bool → !inside
let mask = !0xFFu32;           // u32  → ~0xFFu
```
