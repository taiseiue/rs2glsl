...
use super::{Tail, TypeEnv, FuncRegistry};
        ...
        match stmt {
            syn::Stmt::Local(local) => {
                ...
                if let syn::Expr::If(if_expr) = init_expr {
                    let ty = infer_block_tail_type(&if_expr.then_branch, env, registry, func_registry)?;
                    env.insert(name.clone(), ty.clone());
                    out.push_str(&format!("{} {name};\n", ty.to_glsl()));
                    out.push_str(&generate_if(if_expr, Tail::Assign(&name.clone()), env, registry, func_registry)?);
                } else {
            ...
            syn::Stmt::Expr(expr, semi) => {
                if let syn::Expr::If(if_expr) = expr {
                    out.push_str(&generate_if(if_expr, Tail::Discard, env, registry, func_registry)?);
...
pub(super) fn generate_if(
    ...
    let else_str = match &if_expr.else_branch {
        ...
        Some((_, else_expr)) => match else_expr.as_ref() {
            ...
            syn::Expr::If(nested) => {
                format!(" else {}", generate_if(nested, tail, env, registry, func_registry)?)
            }
