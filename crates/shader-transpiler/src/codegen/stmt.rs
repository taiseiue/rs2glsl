        ...
        match stmt {
            syn::Stmt::Local(local) => {
                ...
                if let syn::Expr::If(if_expr) = init_expr {
                    let ty = infer_block_tail_type(&if_expr.then_branch, env, registry)?;
                    env.insert(name.clone(), ty.clone());
                    out.push_str(&format!("{} {name};\n", ty.to_glsl()));
                    out.push_str(&generate_if(if_expr, Tail::Assign(&name.clone()), env, registry)?);
                } else {
                    let (expr_str, ty) = generate_expr(init_expr, env, registry)?;
                    env.insert(name.clone(), ty.clone());
            ...
            syn::Stmt::Expr(expr, semi) => {
                if let syn::Expr::If(if_expr) = expr {
                    out.push_str(&generate_if(if_expr, Tail::Discard, env, registry)?);
                } else if is_last && semi.is_none() {
                    let (expr_str, _) = generate_expr(expr, env, registry)?;
                    let line = match tail {
...
pub(super) fn infer_block_tail_type(
    ...
    match tail {
        syn::Stmt::Expr(expr, None) => Ok(generate_expr(expr, env, registry)?.1),
...
pub(super) fn generate_if(
    ...
    let (cond_str, _) = generate_expr(&if_expr.cond, env, registry)?;
    let then_body = generate_block(&if_expr.then_branch, env, registry, tail)?;

    let else_str = match &if_expr.else_branch {
        None => String::new(),
        Some((_, else_expr)) => match else_expr.as_ref() {
            syn::Expr::Block(b) => {
                let body = generate_block(&b.block, env, registry, tail)?;
                format!(" else {{\n{body}}}")
            }
            syn::Expr::If(nested) => {
                format!(" else {}", generate_if(nested, tail, env, registry)?)
            }
